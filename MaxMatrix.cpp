/*
 * MaxMatrix
 * Version 2.0 Mai 2022
 * Copyright 2013 Oscar Kin-Chung Au /2022 N-Mrqt
 */
...
void MaxMatrix::rotate(const byte* charOrigin)
{
  int i, j, val;

  charRotate[0] = charOrigin[0];
  charRotate[1] = charOrigin[1];
  for (i = 2; i < 10; i++) {
  charRotate[i] = 0;
  }
  for (i = 2; i < 10; i++) {
    for (j = 0; j < 8; j++) {
      val = ((charOrigin[i] >> j) & 1);
      charRotate[7 - j + 2] |= (val << i - 2);
    }
  }
}
	...
	if (h == 8 && y == 0)
		...
		for (int i=0; i<w; i++)
			for (int j=0; j<h; j++)
			{
				...
				int r = y + j;
				if (c>=0 && c<80 && r>=0 && r<8)
					setDot(r, c, bitRead(charRotate[i+2], j));
			}
}
